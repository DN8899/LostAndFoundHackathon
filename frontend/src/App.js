    ...
    return (
        <Router>
            <Box sx={{ display: 'flex', height: '100vh' }}>
                ...
                <Drawer variant="permanent" sx={{ width: 240, flexShrink: 0, '& .MuiDrawer-paper': { width: 240, backgroundColor: '#ffaa00', color: 'white' } }}>
                    <Toolbar>
                        <Typography variant="h6">Lost and Found</Typography>
                    </Toolbar>
                    <List>
                        {['Home', 'Feed', 'Profile', 'Settings'].map((text) => (
                            <ListItem button key={text} component={Link} to={text === 'Profile' ? '/profile' : '/'}>
                ...
                <Box component="main" sx={{ flexGrow: 1, p: 3, backgroundColor: '#f7b267', overflow: 'auto' }}>
                    <AppBar position="static" color="default" sx={{ mb: 3, backgroundColor: 'white', boxShadow: 'none', borderBottom: '1px solid #e0e0e0' }}>
                        <Toolbar sx={{ justifyContent: 'flex-end' }}>
                            <Button variant={isLoggedIn ? 'contained' : 'outlined'} color={isLoggedIn ? 'error' : 'primary'} onClick={isLoggedIn ? handleLogout : toggleDialog}>
                ...
                <Dialog open={isDialogOpen} onClose={toggleDialog} maxWidth="xs" fullWidth>
                    ...
                    <DialogContent>
                        <TextField margin="dense" label="Email" type="email" fullWidth variant="outlined" value={email} onChange={(e) => setEmail(e.target.value)} sx={{ mb: 2 }} />
                    ...
                    <DialogActions>
                        ...
                        <Button onClick={isCreatingAccount ? handleCreateAccount : handleLogin} variant="contained" color="primary">
                    ...
                    <Box textAlign="center" p={2}>
                        <Typography variant="body2">
                            ...
                            <Button onClick={() => setIsCreatingAccount(!isCreatingAccount)} color="primary" size="small">
            ...
            <Routes>
                <Route path="/profile" element={<ProfilePage user={user} updateUserInfo={updateUserInfo} />} />
            </Routes>
        </Router>
    );
};
